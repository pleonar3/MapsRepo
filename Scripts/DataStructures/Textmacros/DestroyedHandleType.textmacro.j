//! textmacro DestroyedHandleTypeArray takes Type, type, DestroyFunc, wantDestroy
...
public struct $Type$Vector extends $Type$Array
    public boolean wantDestroy = $wantDestroy$
    ...
    public method flush takes nothing returns nothing
        local integer i = 0
        local integer loopEnd = getCount()
        if wantDestroy then
            loop
                exitwhen i == loopEnd
                debug call BJDebugMsg("Calling $DestroyFunc$ on $type$, type: $Type$Vector")
                call $DestroyFunc$(getAtIndex(i))
                set i = i + 1
            endloop
        endif
        call super.flush()
    ...
    public method flush takes nothing returns nothing
        ...
        if wantDestroy then
            ...
            loop
                ...
                debug call BJDebugMsg("Calling $DestroyFunc$ on $type$, type: $Type$Set")
                call $DestroyFunc$(cur)
            endloop
        endif
